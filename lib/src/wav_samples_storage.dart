...
class ChannelMapping
{
  ...
  int offsetOutput;
  double scale;
  ChannelMapping(this.fromChannel,this.toChannel,this.offsetSource,this.length,this.offsetOutput,[this.scale = 1.0]);
}
class MixingInfo
{
  IWavSamplesStorage input;
  List<ChannelMapping> channelMappings; // for every output channel, tell which input channel should be used.
...
class Int16Storage extends IWavSamplesStorage {
  ...
  factory Int16Storage.fromBytes(int channels, ByteData data, Endian numEndianess)
  {
    int samplesPerChannel = data.lengthInBytes~/(channels*2);
    List<Int16List> samplesData = List.generate(channels, (index) => Int16List(samplesPerChannel));
    int currentDataOffset = 0;
    for (int s = 0; s < samplesPerChannel; ++s) {
      for (int ch = 0; ch < channels; ++ch) {
        samplesData[ch][s] =
            data.getInt16(currentDataOffset, numEndianess);
        currentDataOffset += 2;
      }
    }
    ...
    for (var m in mixInfo)
    {
      ...
      for (var chm in m.channelMappings)
      {
        ...
        if (scale == 1)
        {
          ...
          {
            outputChannel[chm.offsetOutput+s]=(inputChannel[chm.offsetSource+s]+outputChannel[chm.offsetOutput+s]).clamp(-32768, 32767);
          }
        }
        else
        {
          ...
          for (int s = 0; s< actualLength ;++s)
          {
            outputChannel[chm.offsetOutput+s]=((inputChannel[chm.offsetSource+s]*intScale>>32)+outputChannel[chm.offsetOutput+s]).clamp(-32768, 32767);
          }
        }
      }
    }
  ...
  @override
  Float32Storage convertToFloat32() {
    return Float32Storage(List<Float32List>.generate(samplesData.length,
        (index) => IWavSamplesStorage._int16ListToFloat32(samplesData[index])), samplesPerChannel);
  }
  
  @override
  Int16Storage convertToInt16() {
    return Int16Storage(List<Int16List>.generate(samplesData.length,
        (index) => Int16List.fromList(samplesData[index])), samplesPerChannel);
  }
  
  @override
  Int32Storage convertToInt32() {
    return Int32Storage(List<Int32List>.generate(samplesData.length,
        (index) => IWavSamplesStorage._int16ListToInt32(samplesData[index])), samplesPerChannel);
  }
  
  @override
  Float64Storage convertToFloat64() {
    return Float64Storage(List<Float64List>.generate(samplesData.length,
        (index) => IWavSamplesStorage._int16ListToFloat64(samplesData[index])), samplesPerChannel);
  }
}
...
class Int32Storage extends IWavSamplesStorage {
  ...
  factory Int32Storage.fromBytes32(int channels, ByteData data, Endian numEndianess)
  {
    int samplesPerChannel = data.lengthInBytes~/(channels*4);
    List<Int32List> samplesData = List.generate(channels, (index) => Int32List(samplesPerChannel));
    int currentDataOffset = 0;
    for (int s = 0; s < samplesPerChannel; ++s) {
      for (int ch = 0; ch < channels; ++ch) {
        samplesData[ch][s] =
            data.getInt32(currentDataOffset, numEndianess);
        currentDataOffset += 4;
      }
    }
  ...
  factory Int32Storage.fromBytes24(int channels, ByteData data, Endian numEndianess)
  {
  ...
  void writeStorage(ByteData data, Endian numEndianess, int bytesPerSample)
  {
    ...
    if (bytesPerSample == 3){
      for (int s = 0; s < samplesPerChannel; ++s) {
        for (int ch = 0; ch < channels; ++ch) {
          data.setInt24(currentDataOffset,samplesData[ch][s]>>8, numEndianess);
          currentDataOffset += 3;
        }
      }
    }
    ...
    for (var m in mixInfo)
    {
      ...
      for (var chm in m.channelMappings)
      {
        ...
        if (scale == 1)
        {
          for (int s = 0; s< actualLength ;++s)
          {
            outputChannel[chm.offsetOutput+s]=(inputChannel[chm.offsetSource+s]+outputChannel[chm.offsetOutput+s]).clamp(-2147483648, 2147483647);
          }
        }
        else
        {
          ...
          for (int s = 0; s< actualLength ;++s)
          {
            outputChannel[chm.offsetOutput+s]=((inputChannel[chm.offsetSource+s]*intScale>>16)+outputChannel[chm.offsetOutput+s]).clamp(-2147483648, 2147483647);
          }
        }
      }
    }
  ...
  Float32Storage convertToFloat32() {
    return Float32Storage(List<Float32List>.generate(samplesData.length,
        (index) => IWavSamplesStorage._int32ListToFloat32(samplesData[index])), samplesPerChannel);
  }
  
  @override
  Int16Storage convertToInt16() {
    return Int16Storage(List<Int16List>.generate(samplesData.length,
        (index) => IWavSamplesStorage._int32ListToInt16(samplesData[index])), samplesPerChannel);
  }
  
  @override
  Int32Storage convertToInt32() {
    return Int32Storage(List<Int32List>.generate(samplesData.length,
        (index) => Int32List.fromList(samplesData[index])), samplesPerChannel);
  }
...
class Float64Storage extends IWavSamplesStorage {
  ...
  factory Float64Storage.fromBytes(int channels, ByteData data, Endian numEndianess)
  {
    int samplesPerChannel = data.lengthInBytes~/(channels*8);
    List<Float64List> samplesData = List.generate(channels, (index) => Float64List(samplesPerChannel));
    int currentDataOffset = 0;
    for (int s = 0; s < samplesPerChannel; ++s) {
      for (int ch = 0; ch < channels; ++ch) {
        samplesData[ch][s] =
            data.getFloat64(currentDataOffset, numEndianess);
        currentDataOffset += 8;
      }
    }
    ...
    for (var m in mixInfo)
    {
      ...
      for (var chm in m.channelMappings)
      {
        ...
        if (scale == 1)
        {
          for (int s = 0; s< actualLength ;++s)
          {
            outputChannel[chm.offsetOutput+s] = (inputChannel[chm.offsetSource+s]+outputChannel[chm.offsetOutput+s]);
          }
        }
        else
        {
          for (int s = 0; s< actualLength ;++s)
          {
            outputChannel[chm.offsetOutput+s] = ((inputChannel[chm.offsetSource+s]*scale)+outputChannel[chm.offsetOutput+s]);
          }
        }
      }
    }
  ...
  Float64Storage convertToFloat64() {
    return Float64Storage(List<Float64List>.generate(samplesData.length,
        (index) => Float64List.fromList(samplesData[index])),samplesPerChannel);
  }
  @override
  Float32Storage convertToFloat32() {
    return Float32Storage(List<Float32List>.generate(samplesData.length,
        (index) => Float32List.fromList(samplesData[index])),samplesPerChannel);
  }

  @override
  Int16Storage convertToInt16() {
    return Int16Storage(List<Int16List>.generate(samplesData.length,
        (index) => IWavSamplesStorage._float64ListToInt16(samplesData[index])),samplesPerChannel);
  }
  
  @override
  Int32Storage convertToInt32() {
    return Int32Storage(List<Int32List>.generate(samplesData.length,
        (index) => IWavSamplesStorage._float64ListToInt32(samplesData[index])),samplesPerChannel);
  }
}
...
class Float32Storage extends IWavSamplesStorage {
  ...
  factory Float32Storage.fromBytes(int channels, ByteData data, Endian numEndianess)
  {
    int samplesPerChannel = data.lengthInBytes~/(channels*4);
    List<Float32List> samplesData = List.generate(channels, (index) => Float32List(samplesPerChannel));
    int currentDataOffset = 0;
    for (int s = 0; s < samplesPerChannel; ++s) {
      for (int ch = 0; ch < channels; ++ch) {
        samplesData[ch][s] =
            data.getFloat32(currentDataOffset, numEndianess);
        currentDataOffset += 4;
      }
    }
  ...
  Int16Storage convertToInt16() {
    return Int16Storage(List<Int16List>.generate(samplesData.length,
        (index) => IWavSamplesStorage._float32ListToInt16(samplesData[index])),samplesPerChannel);
  }
  
  @override
  Int32Storage convertToInt32() {
    return Int32Storage(List<Int32List>.generate(samplesData.length,
        (index) => IWavSamplesStorage._float32ListToInt32(samplesData[index])),samplesPerChannel);
  }
}
