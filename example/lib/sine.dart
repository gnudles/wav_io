...
int main(List<String> arguments) {
  final parser = ArgParser()
    ...
    ..addOption('format',
        defaultsTo: 'i16', allowed: ['u8', 'i16', 'i24', 'i32', 'f32', 'f64']);
  ...
  if (duration <= 0 || duration > 60) {
    print(
        "invalid duration. should be a positive number lower than 60 (seconds)");
    return -1;
  }
  int samplerate = int.tryParse(parsingResults['samplerate'] as String) ?? 0;
  if (samplerate < 256 || samplerate > 192000) {
    print(
        "invalid samplerate. should be a positive number larger than 441 and lower than 192,000");
    return -1;
  }
  double frequency = double.tryParse(parsingResults['freq'] as String) ?? 0;
  if (frequency < 5 || frequency > 22000) {
    print(
        "invalid frequency. should be a positive number larger than 4 and lower than 22,000");
    return -1;
  }
  int samples = (samplerate * duration).floor();
  Float32Storage storage = Float32Storage(samples, 1);
  Float32List data = storage.samplesData[0];
  for (int i = 0; i < samples; ++i) {
    data[i] = 0.5 * sin((i / samplerate) * frequency * 2 * pi);
  }

  WavContent<Float32Storage> floatWav = WavContent<Float32Storage>(
      WavFormat(1, samplerate, 4, 24, 32, FormatType.float32,
          channelMask: SPEAKER_FRONT_CENTER),
      StorageType.float32,
      storage,
      info: ListInfo("sine", "wav_io", "computer", "2023",
          "generated by wav_io dart package", "sinusidal", "1"));
  late IWavContent result = floatWav.to(parsingResults['format']);
  File(parsingResults['output'])
      .writeAsBytesSync(saveWav(result).buffer.asUint8List());
  return 0;
}
